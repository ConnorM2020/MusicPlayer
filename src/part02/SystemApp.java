...
import part01.Genre;
/**
 * All functionality of part 01 and more for part 02
 * of insert Coin, view Credits and set Cost per credit
 * @author Connor Mallon
 */
public class SystemApp {
	static String[] menuOptions = { "Select from Full List", "Select Tune by Artist", "Select Tune by Genre",
			"Add new Tune", "Display Top 10", "Insert Coin into Jukebox", "View Jukebox details", "Switch Off",
			"Exit" };
	...
	static Jukebox myMusic = new Jukebox();
	...
	/**
	 * Allow the user to view all details within the Jukebox, the number of coins
	 * currently held The cost for each credit and the credits the user currently
	 * has.
	 */
	private static void viewDetails() {
		System.out.println("Current credits: " + myMusic.getCredits());
		if (myMusic.getCostPerCredit() == 0) {
			System.out.println("The Cost Per Credit has not yet been set.");
		} else {
			System.out.println("Cost (pence) for each credit: " + myMusic.getCostPerCredit() + "p");
		}
		System.out.println("Current (pence) Coins in Jukebox : " + myMusic.getTotalCoins() + "p\n");
	}

	/**
	 * Serves two functionalties, Allowing to insert a coin, and to call to
	 * setCostPer Credit Will first call to setCostPerCredit and then asks if user
	 * wishes to enter a coin 1 for yes, 2 for no (if no, return to menu)
	 */
	private static void jukeBoxCoins() {
		if (myMusic.getCostPerCredit() > 0) {
			String[] coinOptions = { "10p", "20p", "50p", "£1.00", "£2.00" };
			Menu coinInsert = new Menu("Please insert a coin: ", coinOptions);

			int coin = coinInsert.getUserChoice();
			int convertCoin = 0;
			...
			// Now insert coins
			myMusic.insertCoin(convertCoin);

		} else {
			setCostPerCredits();
			System.out.println("\nDoes the user want to now insert a coin?\n1: for Yes 2: for No.\n>");
			...
			do {
				try {
					...
					if (input == 1) {
						jukeBoxCoins(); // Call back to same function - costPerCredit will be above 0
						ok = true;
					} else if (input == 2) {
...
//		Menu jukeboxOptions = new Menu("Jukebox Options",options);
//		int choice = jukeboxOptions.getUserChoice();
		}
	}

	/**
	 * Allows the user to set the coins required to buy each credit 
	 * For example: set to £2.00 - then user will need £2.00 each time to buy a credit
	 */
	private static void setCostPerCredits() {
		String[] coinOptions = { "10p", "20p", "50p", "£1.00", "£2.00" };
		Menu jukeBoxMenu = new Menu("Set cost per credit", coinOptions);

		int choice = jukeBoxMenu.getUserChoice();

		String storeChoice = coinOptions[choice - 1];
		switch (choice) {
		case 1:
			myMusic.setCostPerCredit(10);
			break;
		case 2:
			myMusic.setCostPerCredit(20);
			break;
		case 3:
			myMusic.setCostPerCredit(50);
			break;
		case 4:
			myMusic.setCostPerCredit(100);
			break;
		case 5:
			myMusic.setCostPerCredit(200);
			break;
		}
		if (myMusic.getCostPerCredit() <= 50) {
			System.out.println("The Cost for each credit: " + myMusic.getCostPerCredit() + "p");
		} else {
			System.out.println("The Cost for each credit: " + storeChoice);
		}
	}
	 ...
	 * "title " by "artist"
	 */
	private static String[] selectFromFullList() {
	...
	private static String selectArtist() {
		...
		String[] refinedArtists = removeDuplicates(sortedArtists); // then remove duplicates that appear
	...
	private static void displayTopTen() {
		 ...
		 */
		for (int i = 0; i < fullList.length; i++) { // extract play count
			split = fullList[i].split(",");
			title = split[1].trim();
			strPlayCount = split[5].trim();
	...
	private static String[] orderPlayCount(String[] array) {
		...
		int index = 0;
		int numberOfSongs = 0; // Counts number created
		 ...
		 * first check number of songs If total number of songs is equal or greater than
		 * 10
		 */
		if (order.length >= 10) {
			for (int i = 0; i < 10; i++) {
	...
	private static boolean switchOff() {
		...
		boolean ok = false;
		System.out.println("Does the User want to switch the Jukebox off?");
	...
	/**
	 * Allows the user to switch the MP3 player on by input 1 to switch on else if
	 * input = 2, then switch off MP3 player
	 * 
	 * @param switchNumber - checks the number of times the user has "switched off"
	 * @param -            displaying "MP3 is already off" it has already been
	 *                     switched off
	 * @return status - if = switchOff() then will switch off MP3 player
	 */
	private static boolean switchOn(int switchOnNumber) {

		boolean status = false;
		boolean ok = false;
		System.out.println("Does the user want to switch Jukebox on?");
			...
			try {
				...
				if (input == 1) {
					...
					if (switchOnNumber == 0) {
						status = myMusic.switchOff();
						switchOnNumber++;
					} else if (switchOnNumber > 0) {
						System.out.println("Jukebox is already off.");
					}
	...
	/**
	 * Allow the user to select a tune by a specific genre First displaying all
	 * genres, where the user will chose from, then retrieves all tunes that
	 * correspond to that specific genre For example: input genreType: Rock and Roll
	 * - returns all Rock and Roll Tunes
	 */
	...
	/**
	 * User can add their own specific tunes - by typing in a title, artist,
	 * duration and genre It wil then add the new tune to the list Try catch
	 * employed to ensure an invalid data input is rejected
	 */
	...
	/**
	 * Some hard coded tunes added to the system - can be accessed through any part
	 * of the program The user can view the Title, Artist and Genre of tune
	 */
	private static void addSomeTunes() {
		...
		myMusic.addtune("Unholy War", "Jacob Banks", 320, Genre.POP);
		...
		myMusic.addtune("Olympia", "Sundra Karma", 310, Genre.DANCE);
		myMusic.addtune("Redbone", "Childish Gambino", 350, Genre.JAZZ);
